...
const bigNum = nums.find((ele) => ele > 100);
console.log(bigNum);
...
const ordered = people.sort((by1,by2) => by1.birthYear > by2.birthYear ? 1 : -1);
...
const earlyModel = cars.filter((item) => item.make < 2000);
console.log(earlyModel);
...
nums.forEach(num => console.log(num > 1000 ? `Thats a big number!` : `This number isnt so big...`))
...
people1.filter(person => person.name.length > 5)
...
console.log(people1.filter(person => person.age > 20 && person.age < 35).sort((a,b) => a.age - b.age));
...
console.log(food.map(item => +item.exp))
